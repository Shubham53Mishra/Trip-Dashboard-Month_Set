...
<?php
// Assuming $conn is already defined and connected elsewhere in your application

// Query to fetch all necessary data
$sql = "SELECT trans_id, drop_date, amount_paid, rent FROM 	trip"; // Replace with your actual table name
...
$sno = 1;
while ($row = $result->fetch_assoc()) {
    $drop_date_current_month = $row["drop_date"];
    $trans_id = $row["trans_id"];
    $paid_rent = $row["amount_paid"];
    $rent = $row["rent"];

    // Compare drop_date with next_booking_date and store the greater value
    $greater_date = ($drop_date_current_month > $next_booking_date) ? $drop_date_current_month : $next_booking_date;
    ...
    // echo "Drop Date: " . $drop_date_current_month . "\n";
    // echo "Next Booking Date: " . $next_booking_date . "\n";
    // echo "Greater Date: " . $greater_date . "\n";
    echo "Status: " . $status . "\n\n";
}

// Assuming the connection will be closed elsewhere
?>
