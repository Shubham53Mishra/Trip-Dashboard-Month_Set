...
include "common/navbar.php"; // Fixed missing closing quote
...
$sql = "SELECT trip.trans_id, trip.drop_date, trip.pickup_date, trip.customer_name, trip.customer_mobile, trip.bikes_id, trip.rent, trip.amount_paid, trip.vendor_email_id, trip.comment
        FROM trip
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($insert_stmt->execute()) {
        // Update the latest comment in the trip table
        $sql_update_trip = "UPDATE trip SET comment = ? WHERE trans_id = ?";
        $update_stmt = $conn->prepare($sql_update_trip);
        if (!$update_stmt) {
            die("Update preparation failed: " . $conn->error);
        }
        $update_stmt->bind_param('ss', $comment, $trans_id);
        $update_stmt->execute();
