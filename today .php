    ...
    <?php if ($result->num_rows > 0): ?>
        <table class="table" style="margin-top: 0;">
            ...
            while ($row = $result->fetch_assoc()):
                ...
                // Ensure $drop_status is initialized
                $drop_status = ''; // Initialize with a default value

                // Convert date strings to timestamps for arithmetic operations
                $current_date_timestamp = strtotime($current_date);
                $drop_date_current_month_timestamp = strtotime($drop_date_current_month);

                // Determine row background color
                $rowStyle = ''; // Default style

                // Condition 1: Total rent = Paid rent => Light Green
                if ($paid_rent == $rent) {
                    $rowStyle = 'style="background-color: lightgreen;"';
                }
                // Condition 2: Drop status of latest booking = done => Light Blue
                else if ($drop_status == 'done') {
                    $rowStyle = 'style="background-color: lightblue;"';
                }
                // Condition 3: (Current date - Drop date > 3 days) and (Total rent not equal to Paid rent) => Light Red
                else if (($current_date_timestamp - $drop_date_current_month_timestamp > 3 * 24 * 60 * 60) && ($paid_rent != $rent)) {
                    $rowStyle = 'style="background-color: lightcoral;"'; // lightcoral is a shade of red
                }
                // Condition 4: (Current date - Drop date is between 0 to 3 days) and (Total rent not equal to Paid rent) => Yellow
                else if (($current_date_timestamp - $drop_date_current_month_timestamp >= 0 && $current_date_timestamp - $drop_date_current_month_timestamp <= 3 * 24 * 60 * 60) && ($paid_rent != $rent)) {
                    $rowStyle = 'style="background-color: lightyellow;"';
                }
            ?>
